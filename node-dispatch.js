...
//   Nothing bad happens if you have a line that doesn't make sense; the server just prints a warning message.
    ...
    require('fs').watchFile(process.argv[2], function () {
      require('fs').readFile(process.argv[2], 'ascii', function (err, data) {err ? console.log(err) : read_routing_table(data)})});
    ...
    require('http').createServer(function (req, res) {
      ...
      parts ? routing_table[parts[1]] ?
        (request = require('http').createClient(Number(routing_table[parts[1]].split(/:/)[1]), routing_table[parts[1]].split(/:/)[0]).
         ...
         req.on('end',  function     () {request.end()})) : console.log('Application not found: ' + parts[1]) : console.log('Invalid URL: ' + req.url)}).listen(8080, '0.0.0.0');

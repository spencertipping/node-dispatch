...
//   Nothing bad happens if you have a line that doesn't make sense; the server just prints a warning message.
    ...
    var routing_table      = {},
        read_routing_table = function (contents) {var lines = (contents || '').replace(/\#.*$/g, '').split(/\n\s*/);
                                                  routing_table = {};
...
//   is read as the root folder of the URL.
    ...
    require('http').createServer(function (req, res) {
      ...
      parts ? routing_table[parts[0]] ?
        (request = require('http').createClient(Number(routing_table[parts[0]].split(/:/)[1]), routing_table[parts[0]].split(/:/)[0]).
                                        request(req.type, parts[1]),
         ...
         req.on('end',  function     () {request.end()})) : console.log('Application not found: ' + parts[0]) : console.log('Invalid URL: ' + req.url)}).listen(8080, '0.0.0.0');
